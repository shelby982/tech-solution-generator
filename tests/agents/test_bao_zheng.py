...
对 LLM 客户端的 monkeypatch 走 ``agents.bao_zheng.generate_oneshot_openai``。
"""
...
from agents.bao_zheng import AGENT_NAME, BaoZhengAgent
...
async def test_review_returns_finding_per_block(monkeypatch):
    ...
    agent = BaoZhengAgent(configs_provider=_fake_configs)
    ...
    for finding in results.values():
        ...
        assert finding.agent == AGENT_NAME == "bao_zheng"
...
async def test_review_returns_empty_when_no_configs():
    ...
    agent = BaoZhengAgent(configs_provider=_no_configs)
    blocks = {
    ...
    assert results["s1"].agent == "bao_zheng"
...
async def test_review_parses_valid_response(monkeypatch):
    ...
    monkeypatch.setattr("agents.bao_zheng.generate_oneshot_openai", fake_oneshot)
...
async def test_review_isolates_block_failure(monkeypatch):
    ...
    monkeypatch.setattr("agents.bao_zheng.generate_oneshot_openai", flaky_oneshot)
...
async def test_review_emits_per_block_events(monkeypatch):
    ...
    assert len(starts) == 2
    assert len(dones) == 2
    assert all(p[1]["agent"] == "bao_zheng" for p in events)
