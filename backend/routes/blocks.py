    ...
    async with get_db() as db:
        ...
        try:
            if config.provider in OPENAI_COMPATIBLE_PROVIDERS:
                suggestion = await _generate_oneshot_openai(config, system_prompt, user_prompt, max_tokens=2000)
            else:
                suggestion = await _generate_oneshot_claude(config, system_prompt, user_prompt, max_tokens=2000)
        except Exception as exc:
